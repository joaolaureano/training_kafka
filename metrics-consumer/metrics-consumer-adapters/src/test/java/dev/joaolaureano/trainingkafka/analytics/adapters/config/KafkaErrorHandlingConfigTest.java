...
class KafkaErrorHandlingConfigTest {
    ...
    void publicaNaDeadLetterPorPadrao() {
        contextRunner.run(context -> {
            ...
            assertThat(context.getBean("ordersDeadLetterTopic", NewTopic.class).name())
    ...
    void sufixoEParticoesSaoConfiguraveis() {
        contextRunner
                ...
                .run(context -> {
                    NewTopic topic = context.getBean("ordersDeadLetterTopic", NewTopic.class);
                    assertThat(topic.name()).isEqualTo("orders.dead");
