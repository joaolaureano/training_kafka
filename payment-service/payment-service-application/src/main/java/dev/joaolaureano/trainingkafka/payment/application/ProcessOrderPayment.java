 ...
 *
 * Nada é publicado aqui. O desfecho e o evento vão para o banco no mesmo commit,
 * e o relay do outbox entrega depois — é o que garante que um pagamento resolvido
 * nunca fique com o resultado preso dentro do processo.
 */
public final class ProcessOrderPayment {
    ...
    public ProcessOrderPayment(PaymentRepository payments, PaymentGateway gateway, Clock clock) {
    ...
    public void handle(String orderId, String customerId, BigDecimal amount, String correlationId) {
        ...
        if (result.approved()) {
            payment.approve(clock.instant(), correlation);
        } else {
            payment.fail(clock.instant(), result.reason(), correlation);
        }
